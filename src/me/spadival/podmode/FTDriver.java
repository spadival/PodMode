...
enum FTDICHIPTYPE {
	FT232RL, FT2232C, FT232H, FT2232D, FT2232HL, FT4232HL, FT230X, CDC, PL2303;
}
...
public class FTDriver {
	 ...
	 * 
	 * If true then checking USB read packet loss and displaying messages to
	 * Logcat
	 * 
	...
	private static final UsbId[] IDS = {
			...
			new UsbId(0x067B, 0x2303, 1, 1, FTDICHIPTYPE.PL2303) // Not a FTDI
																	// chip, but
																	// added
																	// here for
																	// convenience
	...
	public static final int WRITEBUF_SIZE = 64;
	...
	public boolean begin(int baudrate) {
		for (UsbDevice device : mManager.getDeviceList().values()) {
			...
			if ((device.getVendorId() == 0x067b && device.getProductId() == 0x2303)
	...
	public int read(byte[] buf, int channel) {
		...
		if (buf.length <= mReadbufRemain) {
			if (!mReadPakcetChecker) {
				 ...
				 * USB read packet loss checker
				 * 
				 * check: read byte is 01234567890123456789012... requirement :
				 * microcomputer send 01234567890123456789012... to Android
				 */
		...
		if (mSelectedDeviceInfo.mType == FTDICHIPTYPE.PL2303) {
			int packetSize = mFTDIEndpointIN[channel].getMaxPacketSize();
	//		Log.d("FTDriver", "Buffer size " + String.valueOf(packetSize));
	//		packetSize =  64;
			byte[] readBuf = new byte[packetSize];
			len = mDeviceConnection.bulkTransfer(mFTDIEndpointIN[channel],
					readBuf, readBuf.length, 0); // RX

			if (len > 0) {
				
				
				System.arraycopy(readBuf, 0, mReadbuf, 0, len);
			...
			len = mDeviceConnection.bulkTransfer(mFTDIEndpointIN[channel],
					mReadbuf, mReadbuf.length, 2000000000); // RX
	...
	public int write(byte[] buf, int length, int channel) {
		...
		while (offset < length) {
			...
			if (mSelectedDeviceInfo.mType == FTDICHIPTYPE.PL2303) {
				...
				try {
					actual_length = mDeviceConnection
							.bulkTransfer(mFTDIEndpointOUT[channel], write_buf,
									write_size, 0);
				} catch (NullPointerException e) {
					actual_length = 0;
				}
	...
	private void initFTDIChip(UsbDeviceConnection conn, int baudrate) {
		...
		if (mSelectedDeviceInfo.mType == FTDICHIPTYPE.PL2303) {
			byte[] buf = new byte[1];
			conn.controlTransfer(0xc0, 0x01, 0x8484, 0, buf, 1, 100);
			conn.controlTransfer(0x40, 0x01, 0x0404, 0, null, 0, 100);
			conn.controlTransfer(0xc0, 0x01, 0x8484, 0, buf, 1, 100);
			conn.controlTransfer(0xc0, 0x01, 0x8383, 0, buf, 1, 100);
			conn.controlTransfer(0xc0, 0x01, 0x8484, 0, buf, 1, 100);
			conn.controlTransfer(0x40, 0x01, 0x0404, 1, null, 0, 100);
			conn.controlTransfer(0xc0, 0x01, 0x8484, 0, buf, 1, 100);
			conn.controlTransfer(0xc0, 0x01, 0x8383, 0, buf, 1, 100);
			conn.controlTransfer(0x40, 0x01, 0, 1, null, 0, 100);
			conn.controlTransfer(0x40, 0x01, 1, 0, null, 0, 100);

			conn.controlTransfer(0x40, 0x01, 2, 0x44, null, 0, 100);

			byte[] initBytes = new byte[7];

			initBytes[0] = (byte) (baudrate & 0xff);
			initBytes[1] = (byte) ((baudrate >> 8) & 0xff);
			initBytes[2] = (byte) ((baudrate >> 16) & 0xff);
			initBytes[3] = (byte) ((baudrate >> 24) & 0xff);

			initBytes[4] = (byte) 0; // stop bit 1
			initBytes[5] = (byte) 0; // parity none
			initBytes[6] = (byte) 8; // data bit 8

			conn.controlTransfer(0x21, 0x20, 0, 0, initBytes, 7, 100);
			conn.controlTransfer(0x21, 0x23, 0x0000, 0, null, 0, 100);
			conn.controlTransfer(0x40, 0x01, 0x0, 0x0, null, 0, 100); // flow
																		// control
																		// none
	 ...
	 *            CH_A CH_B CH_C CH_D
	 * @param flowControl
	 *            FTDI_SET_FLOW_CTRL_NONE FTDI_SET_FLOW_RTS_CTS_HS
	 ...
	 * 
	 * @param parity
	 *            none : FTDI_SET_DATA_PARITY_NONE odd :
	 ...
	 * 
	 * @param stopBits
	 *            1 : FTDI_SET_DATA_STOP_BITS_1 1.5 : FTDI_SET_DATA_STOP_BITS_15
	 ...
	 * 
	 * @param tx
	 *            TX OFF : FTDI_SET_NOBREAK TX ON : FTDI_SET_BREAK
	 * @param channel
	 *            CH_A CH_B CH_C CH_D
	 * @return true : succeed, false : not succeed
	 */
	...
	private boolean setFTDIEndpoints(UsbInterface[] intf, int portNum) {
		...
		if (isCDC) {
			...
			for (int i = 0; i < portNum; ++i) {
				if (mSelectedDeviceInfo.mType == FTDICHIPTYPE.PL2303) {
					for (int j = 0; j < intf[i].getEndpointCount(); j++) {

						if (intf[i].getEndpoint(j).getType() == UsbConstants.USB_ENDPOINT_XFER_BULK) {
							Log.d("FTDriver", "Interface = " + String.valueOf(i));
							if (intf[i].getEndpoint(j).getDirection() == UsbConstants.USB_DIR_IN) {
								epIn = intf[i].getEndpoint(j);
								Log.d("FTDriver", "epIn = " + String.valueOf(j));
							}
							else {
								epOut = intf[i].getEndpoint(j);
								Log.d("FTDriver", "epOut = " + String.valueOf(j));
